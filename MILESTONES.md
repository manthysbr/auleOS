...
- Persistência de arquivo em `/home/gohan/auleOS/workspace/jobs/{job_id}/result.png`
- URL de arquivo servida pelo kernel (`/v1/jobs/{id}/files/{filename}`)
- Sidebar com status real (incluindo `QUEUED`)
- Chat abre automaticamente o stream quando a tool retorna `job_id`
...
### Gaps para fechar M4
- Falta política de nome/versionamento de artefatos por tentativa
- Falta cobertura de testes focada no fluxo de persistência
- Expandir capacidades além de `image.generate` (ex.: `text.generate`) no mesmo pipeline assíncrono
...
### Gaps para fechar M5
- Falta stream consolidado de progresso percentual
...
1. Finalizar M4: adicionar versionamento/nomes de artefatos por tentativa e testes de integração
